...
1. 安装完成后，编辑 `/etc/sudoers` 文件，按如下方式配置：
...
1. 接下来，为 *windows* 用户提供对显示的访问权限，因为 WINE 程序将以该用户身份运行：

```sh
% xhost +local:windows
```

此命令应当添加到登录时或默认图形环境启动时运行的命令列表中。完成上述所有步骤后，配置为 `WINDOWS_USERS` 的用户可以使用以下命令运行程序，使用共享前缀：
