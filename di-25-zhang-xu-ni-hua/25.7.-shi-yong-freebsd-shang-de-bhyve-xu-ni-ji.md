# 25.7.使用 FreeBSD 上的 bhyve 虚拟机

bhyve 是一款 BSD 授权的虚拟化管理程序，自 FreeBSD 10.0-RELEASE 起成为基本系统的一部分。该虚拟化管理程序支持多个操作系统，包括 FreeBSD、OpenBSD、许多 Linux® 发行版以及 Microsoft Windows®。默认情况下，bhyve 提供对串行控制台的访问，而不模拟图形控制台。利用新处理器的虚拟化卸载功能，避免了翻译指令和手动管理内存映射的传统方法。
...
判断 Intel 或 AMD 处理器是否支持 bhyve 的最简单方法是运行 `dmesg` 或查看 **/var/run/dmesg.boot** 文件，寻找 AMD® 处理器 `Features2` 行中的 `POPCNT` 处理器特征标志，或 Intel® 处理器 `VT-x` 行中的 `EPT` 和 `UG`。
...
虚拟机将引导并开始安装。当系统在安装结束时询问是否进入 shell 时，选择 **Yes**。

重启虚拟机。重启虚拟机会导致 bhyve 退出，但 **vmrun.sh** 脚本会在循环中运行 `bhyve` 并自动重新启动。当发生这种情况时，选择引导菜单中的重启选项以跳出循环。现在可以从虚拟磁盘启动虚拟机系统：
...
Linux 虚拟机可以像其他常规 [基于 UEFI 的虚拟机](./25.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md#id-25.7.4.-shi-yong-uefi-gu-jian-yin-dao-bhyve-xu-ni-ji) 一样引导，或者可以使用 Port [sysutils/grub2-bhyve](https://cgit.freebsd.org/ports/tree/sysutils/grub2-bhyve/) 。
...
1. 首先加载内核，然后启动虚拟机。
2. 使用 Port [sysutils/grub2-bhyve](https://cgit.freebsd.org/ports/tree/sysutils/grub2-bhyve/) 加载 Linux® 内核。
...
使用 Port [sysutils/grub2-bhyve](https://cgit.freebsd.org/ports/tree/sysutils/grub2-bhyve/) 从 ISO 镜像加载 Linux® 内核：
...
系统将引导并开始安装。在虚拟机中安装系统后，重启虚拟机。重启虚拟机会导致 bhyve 退出。必须先销毁虚拟机实例才能重新启动：
...
Linux® 将在虚拟机中引导并最终显示登录提示。登录后使用虚拟机。完成后，重启虚拟机以退出 bhyve。销毁虚拟机实例：

```sh
# bhyvectl --destroy --vm=linuxguest
```

## 25.7.4. 使用 UEFI 固件引导 bhyve 虚拟机

除了 `bhyveload` 和 `grub-bhyve`，bhyve 还可以使用 UEFI 固件引导虚拟机。这种选项可能支持其他加载器不支持的客体操作系统。

要使用 bhyve 中的 UEFI 支持，首先需要获取 UEFI 固件镜像。可安装软件包或 Port [sysutils/bhyve-firmware](https://cgit.freebsd.org/ports/tree/sysutils/bhyve-firmware/) 来完成。
...
为了让虚拟机能存储 UEFI 变量，可以将变量文件附加到 `-l` 标志中。请注意，bhyve 会将虚拟机对变量的修改写入指定的变量文件。因此，请确保首先为每个虚拟机创建一个变量模板文件的副本：
...
> **注意**
>
> 某些 Linux 发行版要求使用 UEFI 变量存储其 UEFI 引导文件的路径（例如，使用 `linux64.efi` 或 `grubx64.efi`，而不是 `bootx64.efi`）。因此，建议为 Linux 虚拟机使用变量文件来避免手动修改引导分区文件。

要查看或修改变量文件的内容，可以在主机上使用 [efivar(8)](https://man.freebsd.org/cgi/man.cgi?query=efivar&sektion=8&format=html)。

Port [sysutils/bhyve-firmware](https://cgit.freebsd.org/ports/tree/sysutils/bhyve-firmware/) 还包含支持 CSM 的固件，用于以传统 BIOS 模式引导没有 UEFI 支持的虚拟机：
...
## 25.7.5. 为 bhyve 虚拟机启用图形 UEFI 帧缓存

UEFI 固件支持对于主要以图形为主的操作系统（如 Microsoft Windows®）特别有用。

可以通过添加 `-s 29,fbuf,tcp=0.0.0.0:5900` 标志启用 UEFI-GOP 帧缓存。可以使用 `w=800` 和 `h=600` 配置帧缓存分辨率，并且可以通过添加 `wait` 指示 bhyve 在引导虚拟机之前等待 VNC 连接。帧缓存可以从主机或通过网络通过 VNC 协议访问。此外，可以添加 `-s 30,xhci,tablet` 以实现与主机的精确鼠标光标同步。
...
请注意，在 BIOS 模拟模式下，若控制从固件传递给客体操作系统，帧缓存将停止更新。

## 25.7.6. 创建 Microsoft Windows® 虚拟机
...
> **警告**
>
> 制造商并不支持修改 Windows 安装媒体以在没有 TPM 模块的情况下运行 Windows 虚拟机。采用此类方法前，请评估你的应用场景和使用需求。

## 25.7.7. 在 bhyve 虚拟机中使用 ZFS
...
现代虚拟机监控程序让用户能创建“快照”，该快照包括虚拟机的磁盘、CPU 和内存内容。快照通常可以在虚拟机运行或关闭的情况下创建。然后，可以将虚拟机恢复到快照拍摄时的精确状态。

### 25.7.8.1. ZFS 快照

使用 ZFS 卷作为虚拟机的后端存储可以对虚拟机磁盘做快照。例如：

```sh
zfs snapshot zroot/path/to/zvol@snapshot_name
```

虽然可以在虚拟机运行时为 ZFS 卷创建快照，但请注意，在虚拟机活跃时，虚拟磁盘的内容可能处于不一致的状态。因此，建议在执行此命令之前，先关闭或暂停虚拟机。默认情况下，不支持暂停虚拟机，需要先启用（参见 [内存和 CPU 快照](./25.7.-shi-yong-freebsd-shang-de-bhyve-xu-ni-ji.md#id-25.7.8.2.-nei-cun-he-cpu-kuai-zhao-shi-yan-xing-gong-neng)）

> **警告**
>
> 在虚拟机使用 ZFS zvol 时回滚到快照可能会破坏文件系统内容并导致虚拟机崩溃。所有未保存的数据将丢失，且自上次快照以来的修改可能会被销毁。
>
> 若虚拟机关闭，可能需要第二次回滚以恢复文件系统的可用状态。这将最终丢失自快照以来所做的所有更改。

### 25.7.8.2. 内存和 CPU 快照（实验性功能）

从 FreeBSD 13 开始，bhyve 具有一个实验性的“快照”功能，可以将虚拟机的内存和 CPU 状态转储到文件中，然后暂停虚拟机。稍后可以从快照文件恢复虚拟机。

然而，此功能默认情况下未启用，并且需要从源代码重新编译系统。有关使用自定义选项编译内核的详细说明，请参见 [从源代码构建](../di-27-zhang-freebsd-geng-xin-yu-sheng-ji/27.7.-cong-yuan-dai-ma-geng-xin-freebsd.md)。
...
> **注意**
>
> 在执行以下步骤之前，请确保 **/usr/src** 目录是最新的。请参见 [更新源代码](../di-27-zhang-freebsd-geng-xin-yu-sheng-ji/27.7.-cong-yuan-dai-ma-geng-xin-freebsd.md) 以了解详细操作步骤。
...
然后按照 [从源代码更新 FreeBSD 快速入门部分](../di-27-zhang-freebsd-geng-xin-yu-sheng-ji/27.7.-cong-yuan-dai-ma-geng-xin-freebsd.md) 中的步骤构建并安装基本系统和内核。
...
> **注意**
>
> 向 `--suspend` 提供绝对路径和文件名。否则，bhyve 将把快照数据写入启动 bhyve 时所在的目录。
>
> 确保将快照数据写入安全目录。生成的输出包含虚拟机的完整核心转储，因此可能包含敏感数据（例如密码）！
...
## 25.7.9. 在 Jail 中运行 bhyve

为提高安全性并将虚拟机与主机操作系统隔离，可以将 bhyve 运行在 jail 中。有关 jail 及其安全性好处的详细说明，请参见 [Jails](../di-17-zhang-jail/17.1.-gai-shu.md)。

### 25.7.9.1. 为 bhyve 创建 Jail
...
如果使用 [ZFS 文件系统](../di-23-zhang-z-wen-jian-xi-tong-zfs/23.1.-shi-shi-mo-shi-zfs-yu-zhong-bu-tong.md)，请使用以下命令：
...
如果不使用 ZFS，使用以下命令直接在 jail 目录结构中创建一个磁盘镜像文件：
...
* 使用前缀为 `tap10` 的 **tap** 网络接口。即有效的接口名称将是 `tap10`、`tap100`、`tap101`、… `tap109`、`tap1000` 等。限制对可能的 **tap** 接口名称的访问将防止 jail（从而 bhyve）看到主机和其他 jails 的 **tap** 接口。
...
此示例假设使用类似 `pf` 或 `ipfw` 的防火墙来 NAT 你的 jail 流量。有关实现此功能的更多详细信息，请参见 [防火墙](../di-34-zhang-fang-huo-qiang/34.1.-gai-shu.md) 章节。
...
**示例 5. 使用 ZFS 引导 UEFI 虚拟机操作系统**

如果你希望使用 UEFI 虚拟机操作系统，请首先在 Jail 内安装所需的固件 Port [sysutils/bhyve-firmware](https://cgit.freebsd.org/ports/tree/sysutils/bhyve-firmware/)：
...
以这种方式销毁虚拟机会立即将其杀死。任何未保存的数据将丢失，打开的文件和文件系统可能会损坏。要优雅地关闭虚拟机，请向其 bhyve 进程发送 `TERM` 信号。这会触发虚拟机的 ACPI 关机事件：
...
为在引导时自动启动 bhyve 虚拟机，需要修改一些配置文件。
...
> **警告**
>
> 修改系统的 IP 地址配置可能会终止当前远程连接（例如通过 SSH 连接），导致无法访问系统！在执行这些命令时，请确保采取必要的预防措施以保持对系统的访问，或在本地终端会话中修改。
