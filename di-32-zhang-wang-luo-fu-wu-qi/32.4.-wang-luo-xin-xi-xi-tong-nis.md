...
网络信息系统（NIS）旨在集中管理类似 UNIX® 的系统，如 Solaris™、HP-UX、AIX®、Linux、NetBSD、OpenBSD 和 FreeBSD。NIS 最初被称为 Yellow Pages 或 YP，但由于商标问题，名称发生了更改。这也是 NIS 命令以 `yp` 开头的原因。
...
为了扩展本章中使用的示例，NIS 域将扩展以添加以下用户和系统：
